...
from models.regionvit_unet.attention.attention_variants import AttentionWithRelPos
from models.regionvit_unet.attention.attention2d import LayerNorm2d
...
class ConvAttBlockUp(nn.Module):
    def __init__(self, channels, kernel_size, num_blocks, num_heads, mlp_ratio=1.,
                 qkv_bias=False, qk_scale=None, pool='sc',
                 act_layer=nn.GELU, norm_layer=nn.LayerNorm, drop_path_rate=(0.,), attn_drop_rate=0., drop_rate=0.,
                 cls_attn=True, peg=False):
        super().__init__()
        ...
        if pool:
            tmp.append(Projection(channels, channels, act_layer=act_layer, mode=pool))

        for i in range(num_blocks):
            kernel_size_ = kernel_size
            tmp.append(
                R2LAttentionPlusFFN(channels, channels, kernel_size_, num_heads, mlp_ratio, qkv_bias,
                                    qk_scale,
                                    act_layer=act_layer, norm_layer=norm_layer, drop_path=drop_path_rate[i],
                                    attn_drop=attn_drop_rate, drop=drop_rate,
                                    cls_attn=cls_attn))

        self.block1 = nn.ModuleList(tmp)
        ...
        self.peg = nn.Conv2d(channels, channels, kernel_size=3, padding=1, groups=channels,
                             bias=False) if peg else None

    def forward(self, xs):
        cls_tokens, patch_tokens = xs
        cls_tokens, patch_tokens = self.block1[0]((cls_tokens, patch_tokens))
        out, mask, p_l, p_r, p_t, p_b, B, C, H, W = convert_to_flatten_layout(cls_tokens, patch_tokens, self.ws)
        for i in range(1, len(self.block1)):
            blk = self.block1[i]
...
class PatchExpand(nn.Module):
    def __init__(self, input_resolution, dim, dim_scale=2, norm_layer=nn.LayerNorm):
        ...
        self.dim = dim
        self.expand = nn.ConvTranspose2d(2*dim, dim, kernel_size=2, stride=2)
        self.norm = norm_layer([dim, input_resolution[0]*dim_scale, input_resolution[0]*dim_scale])
...
class FinalPatchExpand_X4(nn.Module):
    def __init__(self, input_resolution, dim, dim_scale=4, norm_layer=nn.LayerNorm):
        ...
        self.input_resolution = input_resolution
        self.dim = dim
        self.expand = nn.ConvTranspose2d(dim, dim, kernel_size=4, stride=4)
        self.norm = norm_layer([dim, input_resolution[0] * dim_scale, input_resolution[0] * dim_scale])
...
class RegionViT(nn.Module):
    ...
    def __init__(self, img_size=224, patch_size=16, in_chans=1, num_classes=2, embed_dim=(768,), depth=(12,),
        ...
        ################################################################################################################
        # create decoder for RegionViT_UNet

        self.layers_up = nn.ModuleList()
        self.upsamples_patch = nn.ModuleList()
        self.upsamples_token = nn.ModuleList()
        self.concat_back_dim_patch = nn.ModuleList()
        self.concat_back_dim_token = nn.ModuleList()
        ...
        self.concat_back_dim_patch.append(nn.Identity())
        self.concat_back_dim_token.append(nn.Identity())
        self.layers_up.append(ConvAttBlockUp(embed_dim[4], kernel_size=kernel_sizes[3], num_blocks=depth[3],
                                drop_path_rate=dpr_,
                                num_heads=num_heads[3], mlp_ratio=mlp_ratio[3], qkv_bias=qkv_bias, qk_scale=qk_scale,
                                pool='s', norm_layer=norm_layer, attn_drop_rate=attn_drop_rate,
                                drop_rate=drop_rate,
                                cls_attn=True, peg=peg))
        dpr_ptr += curr_depth
        curr_depth = depth[2]
        ...
        self.upsamples_patch.append(PatchExpand(input_resolution=(56 // (2 ** (4 - 1)), 56 // (2 ** (4 - 1))),
                                    dim=256, dim_scale=2, norm_layer=norm_layer))
        self.upsamples_token.append(PatchExpand(input_resolution=(8 // (2 ** (4 - 1)), 8 // (2 ** (4 - 1))),
                                              dim=256, dim_scale=2, norm_layer=norm_layer))

        self.concat_back_dim_patch.append(nn.Linear(2 * int(embed_dim[3]), int(embed_dim[3])))
        self.concat_back_dim_token.append(nn.Linear(2 * int(embed_dim[3]), int(embed_dim[3])))
        self.layers_up.append(ConvAttBlockUp(embed_dim[3], kernel_size=kernel_sizes[2], num_blocks=depth[2],
                                drop_path_rate=dpr_,
                                num_heads=num_heads[2], mlp_ratio=mlp_ratio[2], qkv_bias=qkv_bias, qk_scale=qk_scale,
                                pool='s', norm_layer=norm_layer, attn_drop_rate=attn_drop_rate,
                                drop_rate=drop_rate,
                                cls_attn=True, peg=peg))
        ...
        self.upsamples_patch.append(PatchExpand(input_resolution=(56 // (2 ** (3 - 1)), 56 // (2 ** (3 - 1))),
                                    dim=128, dim_scale=2, norm_layer=norm_layer))
        self.upsamples_token.append(PatchExpand(input_resolution=(8 // (2 ** (3 - 1)), 8 // (2 ** (3 - 1))),
                                              dim=128, dim_scale=2, norm_layer=norm_layer))

        self.concat_back_dim_patch.append(nn.Linear(2 * int(embed_dim[2] * 2 ** 1), int(embed_dim[2] * 2 ** 1)))
        self.concat_back_dim_token.append(nn.Linear(2 * int(embed_dim[2] * 2 ** 1), int(embed_dim[2] * 2 ** 1)))
        self.layers_up.append(ConvAttBlockUp(embed_dim[2], kernel_size=kernel_sizes[1], num_blocks=depth[1],
                                drop_path_rate=dpr_,
                                num_heads=num_heads[1], mlp_ratio=mlp_ratio[1], qkv_bias=qkv_bias, qk_scale=qk_scale,
                                pool='s', norm_layer=norm_layer, attn_drop_rate=attn_drop_rate,
                                drop_rate=drop_rate,
                                cls_attn=True, peg=peg))
        ...
        self.upsamples_patch.append(PatchExpand(input_resolution=(56 // (2 ** (2 - 1)), 56 // (2 ** (2 - 1))),
                                    dim=64, dim_scale=2, norm_layer=norm_layer))
        self.upsamples_token.append(PatchExpand(input_resolution=(8 // (2 ** (2 - 1)), 8 // (2 ** (2 - 1))),
                                              dim=64, dim_scale=2, norm_layer=norm_layer))

        self.concat_back_dim_patch.append(nn.Linear(2 * int(embed_dim[1]), int(embed_dim[1])))
        self.concat_back_dim_token.append(nn.Linear(2 * int(embed_dim[1]), int(embed_dim[1])))
        self.layers_up.append(ConvAttBlockUp(embed_dim[1], kernel_size=kernel_sizes[0], num_blocks=depth[0],
                                drop_path_rate=dpr_,
                                num_heads=num_heads[0], mlp_ratio=mlp_ratio[0], qkv_bias=qkv_bias, qk_scale=qk_scale,
                                pool='s', norm_layer=norm_layer, attn_drop_rate=attn_drop_rate,
                                drop_rate=drop_rate,
                                cls_attn=True, peg=peg))

        self.up = FinalPatchExpand_X4(input_resolution=(img_size // 4, img_size // 4), dim_scale=4,
                                      dim=64)
        self.output = nn.Conv2d(in_channels=64, out_channels=self.num_classes, kernel_size=1, bias=False)
    ...
    def forward_features(self, x, detection=False):
        ...
        tokens = self.cls_token(o_x, extra_padding=True)
        ...
        for idx, layer in enumerate(self.layers):
            tokens, x = layer((tokens, x))
            patches.append(x)
            token.append(tokens)

        return o_x, patches, token

    # Dencoder and Skip connection
    def forward_up_features(self, x, patches, token):
        tok, pat = self.layers_up[0]([token[4], patches[4]])
        tok = self.upsamples_token[0](tok)
        pat = self.upsamples_patch[0](pat)
        tok = tok + token[3]
        pat = pat + patches[3]
        tok, pat = self.layers_up[1]([tok, pat])
        tok = self.upsamples_token[1](tok)
        pat = self.upsamples_patch[1](pat)
        tok = tok + token[2]
        pat = pat + patches[2]
        tok, pat = self.layers_up[2]([tok, pat])
        tok = self.upsamples_token[2](tok)
        pat = self.upsamples_patch[2](pat)
        tok = tok + token[1]
        pat = pat + patches[1]
        tok, pat = self.layers_up[3]([tok, pat])
        x = self.up(pat)
    ...
    def forward(self, x):
        x, patches, token = self.forward_features(x)
        x = self.forward_up_features(x, patches, token)
        x = self.output(x)
        return x
